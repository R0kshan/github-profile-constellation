...
Each repo becomes a `<circle>` node positioned around a radial spiral (not a simple circle — uses per-repo `seedrandom` for **deterministic but varied** radius via `repo.size`, `repo.id`, `repo.created_at`, `repo.node_id`). Nodes connected by minimal spanning tree (Prim's algorithm via `flatMap` + `reduce`) using Euclidean distance. If the longest MST edge exceeds 35% of the canvas width, the whole constellation is scaled in (about its center) so all edges stay under the cutoff and the graph is always connected. The constellation is then scaled down (about its center) if its bounding box plus the largest node radius would breach the viewport padding (`viewportPadding`), so nodes never touch the Constellation panel borders. Twinkling animation durations are randomized based on stargazer count and `randNumGen()`.
